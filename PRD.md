...
每个仓库在状态区域内提供 SourceTree 同等语义的分支操作，不新增独立页签：

- 展示当前分支，并将可选项分为“本地分支”和“远端分支”。
- 支持单仓库执行远端刷新，使用 `fetch --all --prune` 更新远端分支及已删除引用。
- 支持切换到已有本地分支。
- 选择远端独有分支时，创建同名本地分支、建立上游跟踪关系并切换过去。
- 分支名称必须由当前仓库真实分支列表校验，不接受任意命令或未识别引用。
- 切换前要求用户确认当前仓库、原分支和目标分支。
- 工作区存在已跟踪、暂存或未跟踪改动时禁止切换；分支切换不得自动 stash、还原或删除用户文件。
- 仓库正在合并、变基、拣选提交或回退提交时禁止切换，并说明需要先完成或中止当前 Git 操作。
- 配置表仓库存在 Excel/WPS 锁文件时禁止切换，并展示占用原因。
- 拉取、远端刷新和分支切换互斥；操作期间禁用重复提交。
- 成功后立即刷新当前分支、上游、提交、工作区状态和分支列表。
- 失败时在页面结果区保留具体原因和 Git 原始输出，不能只显示短暂提示。
